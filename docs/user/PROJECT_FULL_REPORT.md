...
- Каталог `app/` — прежний UI на Streamlit; в корневом README указано, что актуальный продукт — FastAPI + React. Запуск Streamlit возможен для наследия/сравнения, но не является основным контуром разработки.
