...
АНТИ-МЕТА (внутри текста в [PROMPT]): не вставляй в готовый промпт для пользователя буквальные строки `[PROMPT]`, `[/PROMPT]`, `[QUESTIONS]` и т.п. Если пользователь просит «обернуть в теги» — опиши словами или используй плейсхолдер PLACEHOLDER_PROMPT_BLOCK. Служебные теги протокола — только в структуре твоего ответа снаружи, не внутри содержимого [PROMPT]."""
