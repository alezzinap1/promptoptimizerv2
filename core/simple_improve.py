...
def build_simple_improve_system_prompt(
    preset: str,
    custom_meta: str,
    ...
    parts = [BASE_SYSTEM, SHARED_EDITOR_RULES_BLOCK, f"Режим улучшения:\n{snippet}"]
    tm = (target_model or "").strip() or "unknown"
    guidance = get_target_model_guidance_block(tm)
    if guidance:
        parts.append(
            "[Целевая модель] Улучшенный текст промпта должен учитывать особенности модели ниже:\n\n" + guidance
        )
